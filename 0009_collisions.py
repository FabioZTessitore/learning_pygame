...
# so a Box "is like a" Rect
class Box:
    def __init__(self, windowSurface, (left, top, width, height), bgcolor, (dx, dy)):
...
player = Box(windowSurface, (WIDTH-130, 30, 100, 100), BLACK, (0, 0))
...
while not done:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            ...
            dx = getRandomSpeed()
            dy = getRandomSpeed()
            b = Box(windowSurface, (30, 30, size, size), DARK_GREY, (dx, dy))
            boxes.append(b)
